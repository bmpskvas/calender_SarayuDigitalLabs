...
const MyCalendar = ({ user }) => {
  ...
  const fetchEvents = async () => {
    try {
      const response = await axios.post("http://localhost:8000/api/readevent", {
        email: user.email,
      });
  ...
  const handleSaveEvent = async () => {
    ...
    if (currentEvent) {
      await axios.post("http://localhost:8000/api/updateevent", {
      ...
      await axios.post("http://localhost:8000/api/addevent", newEventData);
    }
    fetchEvents();
    handleCloseDialog();
  };
  ...
  const handleDeleteEvent = async (event) => {
    await axios.post("http://localhost:8000/api/deleteevent", {
      title: event.title,
      email: user.email,
    });
    fetchEvents();
